	...
	return BaseController.extend("com.salesbom.culligan.salesbomdisplay.controller.Worklist", {
			...
			var oFunc = function (far, cry, cost) {
									...
									far.read(z, {
										...
										success: function (x) {
											if (x.results.length === 0) {
												...
												that.getView().byId("matidTSC").setValue("");
				...
				Promise.all([pfunc("/MaterialBOMItem"), qfunc("/MaterialBOM"), rfunc("/A_ProductValuation")]).then(function (x) {
					if (x[0] === "" && x[1] === "") {
						...
						// 	ice[i].qty = oNumberFormat.format(ice[i].qty);
						// }
						...
						that.getView().byId("matidTSC").setValue(adder);
		...
		onSalesSearch: function () {
			...
			AP1.read("/A_SalesOrderItem", {
				urlParameters: {
					"$select": "SalesOrder,SalesOrderItem,Material,RequestedQuantity,SalesOrderItemText,SalesOrderItemCategory,ProductionPlant,MaterialByCustomer",
				...
				success: function (x) {
					...
					for (var m = 0; m < x.results.length; m++) {
						disarr.push({
							...
							MaterialByCustomer: x.results[m].MaterialByCustomer,
							StandardCost: "",
							CalculatedStdCost: ""
						});
					}
					...
					if (sPlant === "") {
						sPlant = x.results[1].ProductionPlant;
					}
					$.sap.gplant = sPlant;
					...
					Promise.all([yFunc("/MaterialBOM", "/MaterialBOMItem", "/A_ProductValuation"), xFunc("/A_ProductValuation")]).then(function (
						...
						if (abc[0] === "not1or5") {
							...
							for (var u = 0; u < disarr.length; u++) {
								if (disarr[u].SalesOrderItemCategory === "TAP" || disarr[u].SalesOrderItemCategory === "CBTP") {
									disarr[u].StandardCost = "";
									disarr[u].CalculatedStdCost = "";
								}
								if (parseFloat(disarr[u].StandardCost) === 0 || parseFloat(disarr[u].CalculatedStdCost) === 0) {
									disarr[u].StandardCost = "";
									disarr[u].CalculatedStdCost = "";
								}
							}
							...
							dxlsx = false;
							for (i = 0; i < disarr.length; i++) {
								for (j = 0; j < abc[1].results.length; j++) {
									if (disarr[i].Material === abc[1].results[j].Product) {
										disarr[i].StandardCost = abc[1].results[j].StandardPrice;
										disarr[i].CalculatedStdCost = parseFloat(abc[1].results[j].StandardPrice * disarr[i].RequestedQuantity).toFixed(3);
										j = abc[1].results.length - 1;
				...
				error: function (y) {
					oBusyDialog.close();
					var err = JSON.parse(y.responseText);
					sap.m.MessageBox.show(
						err.error.message.value, {
							icon: sap.m.MessageBox.Icon.WARNING,
							title: "Dear User",
		...
		onNewxls: function () {
			var newoxModel = new sap.ui.model.json.JSONModel();
			var newdata = this.getView().getModel("SAM").getData();
			newoxModel.setData(newdata);
			this.getView().setModel(newoxModel, "XLSX");
			oBusyDialog.close();
			this.onSalesExcelDownload(this);
		},
			...
			var oFunc = function (far, cry, cost) {
				...
				Promise.all([pfunc("/MaterialBOMItem"), rfunc("/A_ProductValuation")]).then(function (x) {
					if (x[0] === "") {
						...
						// 	if (base[u].StandardCost === "" && base[u].CalculatedStdCost === "") {
						// 		base[u].RequestedQuantity = oNumberFormat.format(base[u].RequestedQuantity);
						// 	} else {
						// 		base[u].RequestedQuantity = oNumberFormat.format(base[u].RequestedQuantity);
						// 		base[u].StandardCost = oNumberFormat.format(base[u].StandardCost);
						// 		base[u].CalculatedStdCost = oNumberFormat.format(base[u].CalculatedStdCost);
						// 	}
						// }
